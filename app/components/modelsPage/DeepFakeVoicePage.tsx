...
export default function DeepFakeVoicePage() {
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      formData.append('audio', files[0].file);

      const voiceApi = import.meta.env.VITE_DEEPFAKE_VOICE_API;
  ...
  return (
    <div id="deep-fake-voice" className="w-full max-w-3xl mx-auto py-10 px-4">
      <div className="bg-white rounded-2xl p-8 shadow-lg mt-15">
        <section>
          ...
          <div className="mb-3 mt-3 flex flex-col">
            <label htmlFor="audioInput" className="font-semibold text-blue-500 ms-1">Upload the audio file to check for deep fake voice:</label>
            <h5 className='text-red-500 font-mono ms-1'>Note: provided audio should be of 1 person.</h5>
          </div>
          ...
          <div className={`flex flex-row items-center justify-between p-6 border-2 rounded-lg mb-5 transition-all duration-200 ${isDragging ? 'border-blue-400 bg-blue-50' : 'border-gray-300 bg-gray-100'}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
            <input ref={fileInputRef} type="file" id="file-upload" className="hidden" accept="audio/*" onChange={handleFileChange} />
            ...
            <label htmlFor="file-upload" className="cursor-pointer flex items-center gap-3 flex-1">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-gray-500">
                  Limit 100MB per file • WAV, MP3 formats only
                </p>
              </div>
            </label>
              ...
              {files.map(fileObj => (
                <div key={fileObj.id} className="flex items-center gap-4 p-4 border-2 border-gray-200 rounded-lg bg-white shadow-sm">
                  <div className="relative w-16 h-16 shrink-0 rounded-lg overflow-hidden">
                    <audio controls className="w-full items-center justify-center">
          ...
          {prediction && !prediction.error && prediction.result && (
            <div className={`mb-6 p-6 rounded-lg border-2 ${prediction.result === "The voice is fake (Edited or AI generated!)" ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200'
              }`}>
              <div className="flex items-start gap-3">{prediction.result === "The voice is fake (Edited or AI generated!)" ? (
                <AlertCircle className="w-6 h-6 text-red-600 shrink-0 mt-0.5" />
              ) : (
                <CheckCircle className="w-6 h-6 text-green-600 shrink-0 mt-0.5" />
              )}
                <div className="flex-1">
                  <h3 className={`text-lg font-bold mb-1 ${prediction.result === "The voice is fake (Edited or AI generated!)" ? 'text-red-800' : 'text-green-800'}`}>{prediction.result === "The voice is fake (Edited or AI generated!)" ? '⚠️ Fake Voice' : '✅ Real Voice'}
                  </h3>
                  <p className={`text-sm mb-3 ${prediction.result === "The voice is fake (Edited or AI generated!)" ? 'text-red-700' : 'text-green-700'
                  }`}>{prediction.result === "The voice is fake (Edited or AI generated!)" ? "The voice is fake (Edited or AI generated!)." : "The voice is real (Not edited or not AI generated)."}
                  </p>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-medium ${prediction.result === "The voice is fake (Edited or AI generated!)" ? 'text-red-600' : 'text-green-600'}`}>
                      Confidence:
                    </span>
                    <div className="flex-1 bg-white rounded-full h-2 overflow-hidden max-w-xs">
                      <div className={`h-2 rounded-full transition-all duration-500 ${prediction.result === "The voice is fake (Edited or AI generated!)" ? 'bg-red-500' : 'bg-green-500'}`} style={{ width: `${prediction.confidence || 0}%` }} />
                    </div>
                    <span className={`text-xs font-bold ${prediction.result === "The voice is fake (Edited or AI generated!)" ? 'text-red-700' : 'text-green-700'}`}>
